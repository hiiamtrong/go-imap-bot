...
class ApiClient {
	...
	// Transactions
	async getTransactions(limit = 20, offset = 0): Promise<ApiResponse<Transaction[]>> {
		return this.request<Transaction[]>(`/transactions?limit=${limit}&offset=${offset}`);
	}
	...
	async getSpendingByMonth(year?: number): Promise<ApiResponse<any>> {
		const url = year ? `/statistics/monthly?year=${year}` : '/statistics/monthly';
		return this.request<any>(url);
	}

	async getSpendingByTag(): Promise<ApiResponse<any>> {
		return this.request<any>('/statistics/tags');
	}
}
