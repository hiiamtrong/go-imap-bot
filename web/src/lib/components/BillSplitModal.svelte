<script lang="ts">
	...
	let selectedUsersList = $derived(users.filter((user) => selectedUsers.has(user.id)));
	...
	async function handleSubmit() {
		...
		const splits = Array.from(selectedUsers).map((userId) => ({
			user_id: userId,
			amount:
				splitMode === "equal" ? calculateEqualSplit() : parseFloat(customAmounts[userId] || "0"),
...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
	<div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
		<div class="p-6">
			...
			<div class="mb-6">
				...
				<div class="flex gap-4">
					<label class="flex items-center">
						<input type="radio" bind:group={splitMode} value="equal" class="mr-2" />
						Equal Split
					</label>
					<label class="flex items-center">
						<input type="radio" bind:group={splitMode} value="custom" class="mr-2" />
						Custom Amounts
					</label>
				</div>
			</div>
			...
			<!-- Global Reason -->
			<div class="mb-6">
				<label for="globalReason" class="label mb-2"> Reason for Split (Optional) </label>
				<input
					id="globalReason"
					type="text"
				...
				<p class="text-xs text-gray-500 mt-1">
					This reason will be applied to all users unless they have individual reasons below.
				</p>
			</div>
			...
			{#if selectedUsersList.length > 0}
				<div class="mb-6">
					...
					<div
						class="space-y-3 max-h-60 overflow-y-auto border border-gray-200 rounded-lg p-3 bg-blue-50"
					>
			...
			{#if selectedUsers.size > 0}
				<div class="mb-6 p-4 bg-gray-50 rounded-lg">
					...
					{#if !isValidSplit() && selectedUsers.size > 0}
						<p class="text-red-600 text-sm mt-2">Split amounts must equal transaction amount</p>
					{/if}
				</div>
			{/if}
