<script lang="ts">
	...
	async function loadPendingTransactions() {
		...
		if (response.error) {
			...
			userSummaries = response.data.sort((a, b) =>
			...
			<div class="flex items-center gap-4">
				<label class="flex items-center cursor-pointer">
					...
					<span class="text-gray-700">
						Use "Angry" reminder template
					</span>
				</label>
			</div>
			{#if selectedUsers.size > 0}
				<button onclick={sendReminders} disabled={sending} class="btn btn-primary disabled:opacity-50">
					{sending ? 'Sending...' : `Send Reminders to ${selectedUsers.size} User${selectedUsers.size > 1 ? 's' : ''}`}
				</button>
			{/if}
		</div>
	</div>
						...
						<div class="flex items-start justify-between p-4">
							<label class="flex items-start gap-3 flex-1 cursor-pointer">
								...
								<div class="flex-1">
									...
									<p class="text-sm text-gray-600 mb-1">Email: {summary.user.email}</p>
									<div class="flex gap-4 text-sm text-gray-600">
										<span>Số lượng bill: <strong>{summary.bill_count}</strong></span>
										<span>Tổng tiền: <strong class="text-orange-600">{formatCurrency(summary.total_amount)}</strong></span>
									</div>
									{#if summary.user.whitelist}
										<span class="inline-block px-2 py-1 bg-green-100 text-green-700 text-xs rounded-full mt-2">
			...
			<div class="mt-6 pt-6 border-t border-gray-200">
				...
				<div class="grid grid-cols-3 gap-4 text-center">
					<div class="p-3 bg-blue-50 rounded-lg">
						...
						<p class="text-2xl font-bold text-blue-600">{totalSummary().userCount}</p>
					</div>
					<div class="p-3 bg-orange-50 rounded-lg">
						<p class="text-sm text-gray-600">Số bill</p>
						<p class="text-2xl font-bold text-orange-600">{totalSummary().billCount}</p>
					</div>
					<div class="p-3 bg-green-50 rounded-lg">
						<p class="text-sm text-gray-600">Tổng tiền</p>
						<p class="text-2xl font-bold text-green-600">{formatCurrency(totalSummary().totalAmount)}</p>
					</div>
				</div>
			</div>
		{/if}
